...
{
  typedef typename Arrangement_on_surface_2<GeomeTraits, TopolTraits> Arrangement_2;
  typedef typename Arr_accessor<Arrangement_2>                        Arr_accessor;
                         ...
                         bool should_merge, boost::mpl::true_)
{
  ...
  typedef typename GeomeTraits::Is_on_y_identification_2              Is_on_y_identification_2;
  ...
  Parameter_space_in_y_2 parameter_space_in_y_2 = arr.geometry_traits()->parameter_space_in_y_2_object();
  Is_on_y_identification_2 is_on_y_identification_2 = arr.geometry_traits()->is_on_y_identification_2_object();
  ...
  for (Vertex_iterator vit = arr.vertices_begin(); vit != arr.vertices_end(); ++vit) {
    ...
    accessor.set_vertex_boundary(vit, is_on_y_identification_2(p) ? ARR_LEFT_BOUNDARY : ARR_INTERIOR,
      parameter_space_in_y_2(p));
  }

  typedef std::pair<Halfedge_handle, X_monotone_curve_2> SplitPair;
  std::vector<SplitPair> remaining_curves;
  ...
  for (Edge_iterator eit = arr.edges_begin(); eit != arr.edges_end(); ++eit) {
    ...
    // Note that the curve may be divided, resulting in two reflected x-monotone curves.
    ...
    if (1 < x_monotone_curves.size()) {
      ...
      // Associate the current halfedge (or its twin) with the second reflected curve so we can insert it later.
      X_monotone_curve_2 additional_curve = object_cast<X_monotone_curve_2>(x_monotone_curves.back());
      Halfedge_handle hh = equal_object(eit->source()->point(), eit->curve().source()) ? eit : eit->twin();
  ...
  for (std::vector<SplitPair>::iterator it = remaining_curves.begin(); it != remaining_curves.end(); ++it) {
    ...
    // Split the halfedge at the shared vertex, setting the new inserted curve as the successor of the already set one.
    accessor.split_edge_ex(hh, vh, hh->curve(), it->second);
  }

  // Merge edges sharing a vertex of degree 2, the point of which was reflected from the boundary.
  if (should_merge) {
    std::vector<Vertex_handle> mergeable_vertices;

    // Identify mergeable vertices.
    Are_mergeable_2 are_mergeable_object = arr.geometry_traits()->are_mergeable_2_object();
    for (Vertex_iterator vit = arr.vertices_begin(); vit != arr.vertices_end(); ++vit) {
    ...
    for (std::vector<Vertex_handle>::iterator it = mergeable_vertices.begin(); it != mergeable_vertices.end(); ++it) {
  ...
  for (DEdge_iterator eit = dcel.halfedges_begin(); eit != dcel.halfedges_end(); ++eit) {
    DHalfedge* h = &(*eit);
    DHalfedge* opp_h = h->opposite();
    DVertex* v = h->vertex();
    DVertex* opp_v = opp_h->vertex();

    // Swap targets between h and its opposite, and update their direction accordingly.
    h->set_vertex(opp_v);
    opp_h->set_vertex(v);
  ...
  for (DFace_iterator fit = dcel.faces_begin(); fit != dcel.faces_end(); ++fit) {
    for (DOuter_ccb_iterator out_ccb_it = fit->outer_ccbs_begin(); out_ccb_it != fit->outer_ccbs_end(); ++out_ccb_it) {
      reverse_ccb_chain(*out_ccb_it);
    }
    for (DInner_ccb_iterator in_ccb_it = fit->inner_ccbs_begin(); in_ccb_it != fit->inner_ccbs_end(); ++in_ccb_it) {
...
void reverse_ccb_chain(typename DHalfedge* ccb_halfedge)
{
...
void outer_ccb_to_inner_ccb(typename Dcel& dcel, typename Dcel::Outer_ccb* outer_ccb) {
 ...
 */
template <typename Dcel>
void inner_ccb_to_outer_ccb(typename Dcel& dcel, typename Dcel::Inner_ccb* inner_ccb) {
  typedef typename Dcel::Halfedge  DHalfedge;
  typedef typename Dcel::Face      DFace;
                    ...
                    bool should_merge)
{
  ...
  typedef typename Arr_res::Has_identified_sides_category                            Has_identified_sides_category;
  ...
  reflect_arrangement_impl(arr_res, should_merge, Has_identified_sides_category());
}
